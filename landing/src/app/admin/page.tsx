...
function StatCard({ label, value, accent = false }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className="bg-[#0D1828] border border-[#1E3A5F] rounded-xl p-5">
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#050A14]">
      {/* Top bar */}
      <header className="bg-[#071020] border-b border-[#1E3A5F] px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={load}
            disabled={loading}
            className="flex items-center gap-1.5 text-xs text-[#94A3B8] hover:text-white transition disabled:opacity-50"
            title="Aggiorna"
          >
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-[#94A3B8] mt-1">Gestione iscritti apulia.ai</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          {/* Status filter */}
          <div className="flex bg-[#0D1828] border border-[#1E3A5F] rounded-lg p-0.5 gap-0.5">
          ...
          <select
            ...
            className="bg-[#0D1828] border border-[#1E3A5F] text-[#94A3B8] text-xs rounded-lg px-3 py-2 focus:outline-none focus:border-[#2563EB] cursor-pointer"
          >
          ...
          <div className="relative flex-1 min-w-[200px]">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#1E3A5F]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <input
              ...
              className="w-full bg-[#0D1828] border border-[#1E3A5F] text-white text-xs rounded-lg pl-8 pr-4 py-2 placeholder-[#1E3A5F] focus:outline-none focus:border-[#2563EB] transition"
            />
          </div>
        ...
        {/* Table */}
        <div className="bg-[#0D1828] border border-[#1E3A5F] rounded-2xl overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-20 text-[#94A3B8] text-sm gap-2">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#1E3A5F]">
                    <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Email</th>
                    <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Stato</th>
                    <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Prodotti</th>
                    <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Lingua</th>
                    <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Fonte</th>
                    <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Iscritto il</th>
                    <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#94A3B8]">Confermato</th>
                  </tr>
                </thead>
                    ...
                    <tr key={s.id} className="hover:bg-[#071020]/60 transition">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex flex-wrap gap-1">
                          {(s.products ?? []).map(p => (
                            <span key={p} className="px-2 py-0.5 bg-[#1E3A5F]/60 text-[#7BAAE8] rounded text-[10px] font-semibold uppercase tracking-wider">
                              {p === 'monthly' ? 'mensile' : p}
                            </span>
                          ))}
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-xs text-[#94A3B8] uppercase font-semibold">{s.preferred_language}</td>
                      <td className="px-4 py-3.5 text-xs text-[#94A3B8]">{s.source ?? '—'}</td>
                      <td className="px-4 py-3.5 text-xs text-[#94A3B8] whitespace-nowrap">{formatDate(s.created_at)}</td>
                      <td className="px-4 py-3.5 text-xs text-[#94A3B8] whitespace-nowrap">
                        {s.confirmed_at ? formatDate(s.confirmed_at) : <span className="text-[#1E3A5F]">—</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {/* Export CSV hint */}
        <p className="text-xs text-[#1E3A5F] mt-4 text-right">
