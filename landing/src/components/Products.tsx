...
export default function Products() {
  const { t } = useLanguage()
  const { weekly, monthly, apulia } = t.products
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-7 max-w-6xl mx-auto">
          ...
          >
            ...
            <h3 className="text-xl font-black text-[#0F172A] mb-2">{weekly.name}</h3>
          ...
          >
            ...
            <h3 className="text-xl font-black text-[#0F172A] mb-2">{monthly.name}</h3>
            <p className="text-sm text-[#475569] mb-6 leading-relaxed flex-1">
              {monthly.description}
            </p>
          ...
          {/* Apulia.ai — applied AI (coming soon) */}
          <article
            className="relative flex flex-col rounded-2xl bg-[#F8FAFC] border border-[#2563EB]/30 p-8 transition-all duration-300 group opacity-95"
            aria-label={`${apulia.name} — ${apulia.badge} — prossimamente disponibile`}
          >
            <div
              className="absolute top-0 inset-x-0 h-0.5 rounded-t-2xl bg-gradient-to-r from-[#2563EB]/0 via-[#2563EB] to-[#2563EB]/0"
              aria-hidden="true"
            />
            <ComingSoonRibbon />

            <div className="mb-5 flex items-center justify-between">
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-[#2563EB]/15 text-[#2563EB] border border-[#2563EB]/25">
                {apulia.badge}
              </span>
              <span className="text-sm text-[#475569] font-medium">{apulia.frequency}</span>
            </div>

            <h3 className="text-xl font-black text-[#0F172A] mb-2">{apulia.name}</h3>
            <p className="text-sm text-[#475569] mb-6 leading-relaxed flex-1">
              {apulia.description}
            </p>

            <ul className="space-y-2.5 mb-8" aria-label={`${apulia.name} features`}>
              {apulia.features.map((f, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-[#475569]">
                  <CheckIcon />
                  <span>{f}</span>
                </li>
            ...
            <div className="mt-auto">
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-2xl font-black text-[#0F172A]">{apulia.price}</span>
              </div>
              <p className="text-xs text-[#2563EB]/70 mb-4 font-medium">{apulia.priceNote}</p>
              <button
                type="button"
                disabled
                aria-disabled="true"
